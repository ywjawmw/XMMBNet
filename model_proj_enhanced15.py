...
class FGCN(nn.Module):
    ...
    def fuse_protein_feat(self, protein_sim_feat, protein_scgpt_feat):
        ...
        z_fused, _ = self.pro_view_gate(z_sim, z_sc)  # [N_pro, N_pro]
        return z_fused
...
class Net(nn.Module):
    ...
    def forward(self, enc_graph, dec_graph,
        ...
        if self.merge_type == 'stack':
            ...
            # inner 过一个投影层--》fuse
            drug_evidence_out, drug_beta = self.fuse_drug_inner(drug_sim_out_proj, drug_e_out_proj)
            dis_evidence_out, dis_beta = self.fuse_disease_inner(dis_sim_out_proj, dis_e_out_proj)
            ...
            if self.merge_type == 'fusion':    # intra fuse
                drug_feat, drug_gama = self.fuse_drug_intra(drug_direct_out_proj, drug_evidence_out_proj)
                dis_feat, dis_gama = self.fuse_disease_intra(dis_direct_out_proj, dis_evidence_out_proj)
                pred_ratings = self.decoder(dec_graph, drug_feat, dis_feat)
                weight_list = [drug_beta, dis_beta, drug_gama, dis_gama]
                if train:
                    return pred_ratings, drug_out, drug_evidence_out, dis_out, dis_evidence_out, pro_sim_out, pro_e_dout, pro_e_sout, weight_list
                else:

                    return pred_ratings, drug_feat, dis_feat, weight_list
