...
def projection(args, z: torch.Tensor) -> torch.Tensor:
    fc1 = torch.nn.Linear(args.num_hidden, args.num_proj_hidden1).to(args.device)
    fc2 = torch.nn.Linear(args.num_proj_hidden1, args.num_proj_hidden2).to(args.device)
    fc3 = torch.nn.Linear(args.num_proj_hidden2, args.num_hidden).to(args.device)
    z1 = F.elu(fc1(z))
    z2 = F.elu(fc2(z1))
    return fc3(z2)
...
def LOSS_multi(args, model, z1: torch.Tensor, z2: torch.Tensor, z3: torch.Tensor,
        mean: bool = True, batch_size: int = 0, flag: int = 0):
    if args.projection_train is False:
        h1 = projection(args, z1)
        h2 = projection(args, z2)
        h3 = projection(args, z3)
    else:
        h1 = model.proj_1(z1)
        h2 = model.proj_2(z2)
        h3 = model.proj_3(z3)

    loss= multi_positive_ntxent([h1, h2, h3], args.tau)
    return loss
